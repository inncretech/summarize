var summarizit = new function(object) {
	...
	this.load = function (app_id,thread_id,iFrame_container) {
		$(document).ready(function(){
			$('#'+iFrame_container).html("<iframe name='summarizit-iFrame' width='680' id='summarizit-iFrame' frameBorder='0' scrolling='no' src='http://www.summarizit.com/application/index.php?app_token="+app_id+"&thread_id="+thread_id+"&parent="+encodeURIComponent(window.location.href)+"'></iframe>");
			...
			function onMessage(messageEvent) {
				...
				if (messageEvent.origin == "http://" + guestDomain) {
					...
					if (messageEvent.data["height"])         {$('#'+iFrame_container+" iframe").animate({height:messageEvent.data["height"]+"px"},0);}
				}
			}
