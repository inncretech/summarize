<?php
echo "<div id='follow-div' style='float:right;width:800px;'>";
$prodData=$database->query("SELECT * FROM notifications_follow WHERE uid='".$uid."'");
while ($prodInfo=mysql_fetch_array($prodData)){
	$data = $database->query("SELECT SUM(thumb),idproduct FROM `feedback` WHERE idproduct='".$prodInfo['product']."' GROUP BY `idproduct` ");
	$items = array();
	$aux=0;
	while ($info = mysql_fetch_array($data)){
		$p = $database->query("SELECT * FROM `product` WHERE `idproduct`='".$info['idproduct']."'");
		if (mysql_num_rows($p)>0)
		{
			...
			echo "</div>";
			echo "<div style='min-height:54px;'>".substr($p_info['comment'], 0, 210)."...<a href='result.php?id=".$p_info['idproduct']."'>more</a></div>";
