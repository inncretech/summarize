...
<?php 
if (count($member_data['follow'])==0){
echo '<div class="alert alert-warning" style="margin:0px;"><strong>No products followed at this time.</strong></div>';
}
for ($i = 0; $i < count($member_data['follow']); $i++) {
	$value = $database->product->get($member_data['follow'][$i]['product_id']);
	...
	echo '</div>';
	echo '<button style="float:right;" class="btn btn-danger" onclick="unFollow(this,'.$value['product_id'].');return false;">Unfollow</button></div>';
}
