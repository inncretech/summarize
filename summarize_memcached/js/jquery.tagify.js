...
(function ($) {
	$.widget("ui.tagify", {
		options: {
			delimiters: [44],          // what user can type to complete a tag in char codes: [enter], [comma]
	...
	function addTagEvents(){
			$(".tagify-container").find("span").click(function(){
			var str = $(this).text();
			str = str.substring(0, str.length - 1);
			str = str.replace(' ', '');
			window.location.href="similar.php?tag="+str;

			});
			$(".tagify-container").find("span").mouseover(function(){var tag = $(this); tag.css("cursor","pointer");});
			}
})(jQuery);