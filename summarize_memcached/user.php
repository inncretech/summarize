...
function hideAll(){
	...
	$('#msg-container-send').css('display','none');
...
function fillActivity(user,tag){
  var str = '';
  $('#user-activity').html('<button style="float:right;margin:15px 0 0 5px;" onclick="closeActivity();" class="btn btn-primary start"><i class="icon-star icon-white"></i><span>Close</span></button>');
		
	  $.post("include/getActivity.php", {u: user,t: tag},function(data){
		data = $.parseJSON(data);
		if (data.length==0){str +='<span id="title" >Points earned on feedback likes!</span>';}else
		{
